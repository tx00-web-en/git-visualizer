...
const yargsParser = (typeof window !== 'undefined' && window.yargsParser) || globalThis.yargsParser;
"use strict";
...
ControlBox.prototype = {
  ...
  this.info('`git switch`')
    ...
    'switch': function(args, opts, cmdStr) {
      var parsed = yargs(cmdStr || '', {
        alias: { create: ['c'] },
        string: ['create', 'c']
      })

      var checkoutArgs = parsed._ ? parsed._.slice() : []
      var checkoutOpts = {
        _: checkoutArgs.slice()
      }

      var createValue = parsed.create !== undefined ? parsed.create : parsed.c

      if (Array.isArray(createValue)) {
        createValue = createValue[createValue.length - 1]
      }

      if (createValue) {
        checkoutOpts.b = createValue
      }

      this.checkout(checkoutArgs, checkoutOpts)
    },
